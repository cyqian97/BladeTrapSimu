classdef automaticSTL
    ...
    methods
        function obj = automaticSTL(varargin)
            ...
            % stl_id selects the original stl which we modify. It should be
            % 1 or 2, and the default value is 1.
            p = inputParser;
            addOptional(p,'stl_id',1,@(x) validateattributes(x,{'numeric'},{'scalar'},mfilename,'stl_id',1)); 
            parse(p,varargin{:});
            if p.Results.stl_id~=1 && p.Results.stl_id~=2
                error("stl_id should be 1 or 2, but the current value is %d",p.Results.stl_id);
            end
            ...
            % Use regular expression to get the parameters from the name of
            % the stl file.
