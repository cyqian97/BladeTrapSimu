function [model, result_struct] = SolvePdeBlades3(stlname,pdeoptionsblades,varargin)
%[model, result] = SolvePdeBlades(stlname,,pdeoptionsblades,varargin)
%   Solve pde for blade trap model
%   == Input ==
%   sltname : the name for the slt file which contains the model
%   infromation.
%   pdeoptionsblades : the options for my pde solving functions
...
interp_rate = pdeoptionsblades.interp_rate;
...
dataname = IsExistSolution3(stlname,pdeoptionsblades);
...
if isempty(p_result)
    ...
    for i = 0:2
        for j = [-1,1]
            ...
            yy = temp{mod(-i+1,3)+1};
            zz = temp{mod(-i+2,3)+1};
            if pdeoptionsblades.symmetrize
...
if savedata
    dataname = SavePdeResults3(model,result,stlname,pdeoptionsblades);
end
