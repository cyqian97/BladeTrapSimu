function findfacenameX_func(stlname)
...
% Find Vertex on the edge x = x_max, y = y_max and z < z_max Note that the orientation of the trap affect
...
blade1v = find(eqtol(vertices(:,2),cuboid(1,2),tol)&eqtol(vertices(:,3),blade1z,tol));
...
for i = 1:g.NumVertices
    coord = vertices(i,:);
    if eqtol(coord(1),cuboid(2,1),tol) && eqtol(coord(2),cuboid(2,2),tol) && coord(3)>cuboid(2,3)+tol
        if coord(3)<blade1z
           blade1z  = coord(3);
        end
    end
...
% Find start points in all target electrodes
% (x = 0, z = z0)
blade1v = find(eqtol(vertices(:,2),cuboid(2,2),tol)&eqtol(vertices(:,3),blade1z,tol));
blade1vx = vertices(blade1v,1);
[~,index] = sort(blade1vx);
blade1v = blade1v(index);
elef1 = cell(length(blade1v),1);
