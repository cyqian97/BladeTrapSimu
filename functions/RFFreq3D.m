function [mdl,f,direction] = RFFreq3D(coord_1,coord_2,coord_3,U,varargin)
...
modelfun = @(b,x)b(1) + (1 + b(2).*x(:,1) + b(3).*x(:,1).^2).*(b(4).*x(:,2).*x(:,3) + b(5).*x(:,2).^2 + b(6).*x(:,3).^2);
beta0 = [0 0 0 0 0 0];
...
[V,D] = eig([b(5), b(4)/2; ...
    b(4)/2, b(6)]);
f = ([b(3),D(1,1),D(2,2)]*scale^2*phys.e*2/m).^(1/2)/2/pi;
direction = V;
